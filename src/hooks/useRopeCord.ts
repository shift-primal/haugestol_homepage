...
export function useRopeCord({
    ...
    useEffect(() => {
        ...
        const tick = (time: number) => {
            ...
            if (knobRef.current) {
                ...
                const angleDeg =
                    Math.atan2(
                        kx - renderX(prev, alpha),
                        ky - renderY(prev, alpha)
                    ) *
                    (180 / Math.PI);
            ...
            const stretch =
                Math.hypot(knob.x - ANCHOR_X, knob.y) - config.restLength;
            ...
            if (!draggingRef.current && steps > 0) {
                settledSteps =
                    maxMovement < SETTLE_EPSILON ? settledSteps + steps : 0;
            }
    ...
    const initialKnobAngleDeg =
        -Math.atan2(
            initialKnob.x - initialPrev.x,
            initialKnob.y - initialPrev.y
        ) *
        (180 / Math.PI);
